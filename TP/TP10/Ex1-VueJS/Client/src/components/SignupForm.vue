...
export default {
  ...
  async mounted() {
    ...
    if (data.message == "You can not register again") {
      this.show = true;
    }
  },
  methods: {
    async created() {
      ...
      if (data.status == "true") {
        ...
        if (data.message.keyValue.email) {
          this.msg = data.message.keyValue.email + " already token";
        } else {
          this.msg = data.message.keyValue.username + " already token";
        }
...
.log_in {
  background-color: rgba(109, 165, 114, 0.603);
  width: 50%;
  padding: 1rem;
  border-radius: 2%;
  box-shadow: rgba(134, 226, 175, 0.3) 0px 0px 0px 6px;
}
