  ...
  async mounted() {
    ...
    for (var i = 0; i < this.numOfCate; i++) {
      this.display_item =
        ...
        `<td><button @click="editBtn(${i})">Edit</button><button style="color: red" @click="deleteBtn(${i})">Delete</button></td>` +
        `</tr>`;
    }
...
.container-dashboard .items {
  ...
  width: 100vw;
  height: 92.5vh;
  border: 2px solid  rgb(116, 101, 101);
}
.items .item-1 {
  width: 10%;
  display: flex;
...
li > a {
  border-bottom: 2px solid rgb(88, 78, 78);
  padding: 0.5vh 2.7vw;
}
li > a:focus {
