<template>
  <div class="container-product">
    <div class="header-product">SOCHEATA SHOP</div>
    <div class="box">
      <div class="title">
        ...
        <div class="items-card" v-for="k in arrofItem[render]" :key="k">
          ...
          <table style="width: 100%; text-align: center; color: white">
            <tr v-for="n in numShop[parseInt(k - 1)]" :key="n">
              <td>{{ titleItem.data[render].numShop[parseInt(k - 1)][parseInt(n - 1)].shopName }}</td>
              <td>{{ titleItem.data[render].numShop[parseInt(k - 1)][parseInt(n - 1)].price }}</td>
            </tr>
          </table>
        </div>
      </div>
...
export default {
  ...
  async mounted() {
    ...
    console.log(this.arrofItem[2]);
    console.log(this.titleItem.data);
    for (let j = 0; j < this.arrofItem[2]; j++) {
      this.shop[j] = this.titleItem.data[2].product[j].shop;
...
.container-product .header-product {
  background-color: rgba(25, 73, 177, 0.8);
  border-radius: 10px;
  text-align: center;
  font-size: 20px;
  font-weight: 700;
...
.box .title {
  ...
  width: 19%;
  height: 90vh;
  box-shadow: rgba(255, 255, 255, 0.952) 0px 1px 2px, rgba(0, 0, 0, 0.07) 0px 2px 4px,
...
.items-card {
  ...
  box-shadow: rgba(41, 41, 41, 0.596) 0px 7px 29px 0px;
  border-radius: 8px;
}
a {
  font-family: verdana;
